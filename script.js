...
async function processInitialSearchRequest() {
    ...
    const url = `https://kinopoiskapiunofficial.tech/api/v2.2/films?keyword=${title}&type=${type}&genres=${genre}&page=${pageNumber}`;
...
async function processNextSearchRequest() {
    ...
    if (currentScroll >= availScrollHeight) {        
        ...
        // Формирование URL-адреса
        const url = `https://kinopoiskapiunofficial.tech/api/v2.2/films?order=RATING&type=${type}&keyword=${title}&genres=${genre}&page=${pageNumber}`;
